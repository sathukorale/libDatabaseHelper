...
namespace libDatabaseHelper.classes.sqlce
{
    public class DatabaseEntity
    {
        ...
        public static string GetSelectCommandString(Type type)
        {
            ...
            var fields = obj.GetColumns(true).GetOtherColumns().Where(i => ((TableColumn)i.GetCustomAttributes(typeof(TableColumn), true)[0]).IsRetrievable);
            ...
            foreach (var fieldInfo in fields)
            {
                ...
                if (attr.ReferencedField != null && attr.ReferencedClass != null)
                {
                    ...
                    selectList += "(CASE WHEN " + cInstanceName + "." + fieldInfo.Name + " IN ( SELECT " + attr.ReferencedField + " FROM " +
                                  ...
                                  attr.ReferencedField + " LIMIT 1) ELSE \"\" END) as [" + attr.DisplayName + "]";
                }
                else
                {
                    selectList += cInstanceName + "." + fieldInfo.Name + " as [" + ((attr.DisplayName == null || attr.DisplayName.Trim() == "") ? fieldInfo.Name : attr.DisplayName) + "]";
                }
            }
        ...
        public bool Add()
        {
            ...
            foreach (var field in result.GetOtherColumns().Where(i => ! ((TableColumn)i.GetCustomAttributes(typeof(TableColumn), true)[0]).IsAutogen))
            {
            ...
            foreach (var field in from field in result.GetOtherColumns().Where(i => ((TableColumn)i.GetCustomAttributes(typeof(TableColumn), true)[0]).IsAutogen) let col = ((TableColumn)field.GetCustomAttributes(typeof(TableColumn), true)[0]) where col != null && col.IsAutogen && FieldTools.IsTypeNumber(field.FieldType) select field)
            {
                autogenField = field;
            }
        ...
        public ExistCondition Exist(bool justCheck)
        {
            ...
            var filtered = (justCheck ? result.GetPrimaryKeys() : result.GetPrimaryKeys().Where(i => ! ((TableColumn)i.GetCustomAttributes(typeof(TableColumn), true)[0]).IsAutogen)).ToArray();
        ...
        public ColumnResult GetColumns(bool includeAllColumns)
        {
            ...
            includedFields =
                includedFields.Where(i => ((TableColumn) i.GetCustomAttributes(typeof (TableColumn), true)[0]).IsInTable).ToList();
            if (!includedFields.Any())
            {
                return null;
            }
            ...
            var ids =
                includedFields.Where(
                    i => ((TableColumn) i.GetCustomAttributes(typeof (TableColumn), true)[0]).PrimaryKey).ToList();

            if (!includeAllColumns)
                includedFields =
                    includedFields.Where(
                        i =>
                            ((TableColumn) i.GetCustomAttributes(typeof (TableColumn), true)[0]).IsInTable &&
                            !((TableColumn) i.GetCustomAttributes(typeof (TableColumn), true)[0]).PrimaryKey).ToList();

            if (!includeAllColumns)
                includedFields =
                    includedFields.Where(
                        i => ! ((TableColumn) i.GetCustomAttributes(typeof (TableColumn), true)[0]).IsAutogen).ToList();
        ...
        public void LoadToForm(Control parentControl)
        {
            foreach (Control control in parentControl.Controls)
            {
                ...
                if (info != null)
                {
                    control.Enabled = info.IsEditableWhenLoaded;
                }
                    ...
                    if (control is TextBox)
                    {
                        ...
                        if (info != null)
                        {
                            ...
                            else if ((info.FieldType == typeof (string) || info.FieldType == typeof (String)))
                            {
                                ...
                                if (attr != null)
                                {
                                    if (attr.IsUnique || attr.PrimaryKey || attr.IsAutogen)
                                    {
        ...
        private void CreateDataGridViewHeader(DataGridView grid)
        {
            ...
            foreach (var fieldInfo in fields)
            {
                ...
                var cindex = grid.Columns.Add(fieldInfo.Name, cInfo.DisplayName ?? fieldInfo.Name);

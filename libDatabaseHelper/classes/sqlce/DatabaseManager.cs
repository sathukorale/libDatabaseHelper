    ...
    public class DatabaseManager : GenericDatabaseManager
    {
        ...
        private bool ExecuteNonQuery(DbConnection connection, string commandString, FillArguments argumentFiller = null)
        {
            using (var command = connection.CreateCommand())
            {
                ...
                try
                {
                    ...
                    if (argumentFiller != null)
                    {
                        var dbCommand = command;
                        argumentFiller.Invoke(ref dbCommand);
                    }
        ...
        public override bool CreateTable(Type type)
        {
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                var primaryVariableDeclarations = result.GetPrimaryKeys().Aggregate("", (current, column) => current + ((current == "" ? "" : ", ") + column.Name));
                if (primaryVariableDeclarations != "")
                {
                    primaryVariableDeclarations = ", PRIMARY KEY (" + primaryVariableDeclarations + ")";
                }

                var createStatement = "CREATE TABLE " + obj.GetType().Name + "(" + variableDeclarations + primaryVariableDeclarations + ")";
        ...
        public override GenericDatabaseEntity[] Select(Type type, Selector[] selectors)
        {
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                var selectStatement = "SELECT " + selectFields + " FROM " + obj.GetType().Name + (selectors != null && selectors.Any() ? (" WHERE " + whereStatement) : "");
